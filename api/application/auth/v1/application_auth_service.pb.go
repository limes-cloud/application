// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.34.1
// 	protoc        v4.24.4
// source: api/application/auth/application_auth_service.proto

package v1

import (
	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	emptypb "google.golang.org/protobuf/types/known/emptypb"
	reflect "reflect"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

var File_api_application_auth_application_auth_service_proto protoreflect.FileDescriptor

var file_api_application_auth_application_auth_service_proto_rawDesc = []byte{
	,
}

var file_api_application_auth_application_auth_service_proto_goTypes = []interface{}{
	(*RefreshTokenRequest)(nil),     // 0: application.api.application.auth.v1.RefreshTokenRequest
	(*LogoutRequest)(nil),           // 1: application.api.application.auth.v1.LogoutRequest
	(*emptypb.Empty)(nil),           // 2: google.protobuf.Empty
	(*ListAuthRequest)(nil),         // 3: application.api.application.auth.v1.ListAuthRequest
	(*CreateAuthRequest)(nil),       // 4: application.api.application.auth.v1.CreateAuthRequest
	(*UpdateAuthStatusRequest)(nil), // 5: application.api.application.auth.v1.UpdateAuthStatusRequest
	(*DeleteAuthRequest)(nil),       // 6: application.api.application.auth.v1.DeleteAuthRequest
	(*ListOAuthRequest)(nil),        // 7: application.api.application.auth.v1.ListOAuthRequest
	(*DeleteOAuthRequest)(nil),      // 8: application.api.application.auth.v1.DeleteOAuthRequest
	(*GenAuthCaptchaRequest)(nil),   // 9: application.api.application.auth.v1.GenAuthCaptchaRequest
	(*OAuthLoginRequest)(nil),       // 10: application.api.application.auth.v1.OAuthLoginRequest
	(*EmailLoginRequest)(nil),       // 11: application.api.application.auth.v1.EmailLoginRequest
	(*PasswordLoginRequest)(nil),    // 12: application.api.application.auth.v1.PasswordLoginRequest
	(*EmailRegisterRequest)(nil),    // 13: application.api.application.auth.v1.EmailRegisterRequest
	(*PasswordRegisterRequest)(nil), // 14: application.api.application.auth.v1.PasswordRegisterRequest
	(*EmailBindRequest)(nil),        // 15: application.api.application.auth.v1.EmailBindRequest
	(*PasswordBindRequest)(nil),     // 16: application.api.application.auth.v1.PasswordBindRequest
	(*RefreshTokenReply)(nil),       // 17: application.api.application.auth.v1.RefreshTokenReply
	(*LogoutReply)(nil),             // 18: application.api.application.auth.v1.LogoutReply
	(*AuthReply)(nil),               // 19: application.api.application.auth.v1.AuthReply
	(*ListAuthReply)(nil),           // 20: application.api.application.auth.v1.ListAuthReply
	(*CreateAuthReply)(nil),         // 21: application.api.application.auth.v1.CreateAuthReply
	(*UpdateAuthStatusReply)(nil),   // 22: application.api.application.auth.v1.UpdateAuthStatusReply
	(*DeleteAuthReply)(nil),         // 23: application.api.application.auth.v1.DeleteAuthReply
	(*ListOAuthReply)(nil),          // 24: application.api.application.auth.v1.ListOAuthReply
	(*DeleteOAuthReply)(nil),        // 25: application.api.application.auth.v1.DeleteOAuthReply
	(*GenAuthCaptchaReply)(nil),     // 26: application.api.application.auth.v1.GenAuthCaptchaReply
	(*OAuthLoginReply)(nil),         // 27: application.api.application.auth.v1.OAuthLoginReply
	(*EmailLoginReply)(nil),         // 28: application.api.application.auth.v1.EmailLoginReply
	(*PasswordLoginReply)(nil),      // 29: application.api.application.auth.v1.PasswordLoginReply
	(*EmailRegisterReply)(nil),      // 30: application.api.application.auth.v1.EmailRegisterReply
	(*PasswordRegisterReply)(nil),   // 31: application.api.application.auth.v1.PasswordRegisterReply
	(*EmailBindReply)(nil),          // 32: application.api.application.auth.v1.EmailBindReply
	(*PasswordBindReply)(nil),       // 33: application.api.application.auth.v1.PasswordBindReply
}
var file_api_application_auth_application_auth_service_proto_depIdxs = []int32{
	0,  // 0: application.api.application.auth.v1.Auth.RefreshToken:input_type -> application.api.application.auth.v1.RefreshTokenRequest
	1,  // 1: application.api.application.auth.v1.Auth.Logout:input_type -> application.api.application.auth.v1.LogoutRequest
	2,  // 2: application.api.application.auth.v1.Auth.Auth:input_type -> google.protobuf.Empty
	3,  // 3: application.api.application.auth.v1.Auth.ListAuth:input_type -> application.api.application.auth.v1.ListAuthRequest
	4,  // 4: application.api.application.auth.v1.Auth.CreateAuth:input_type -> application.api.application.auth.v1.CreateAuthRequest
	5,  // 5: application.api.application.auth.v1.Auth.UpdateAuthStatus:input_type -> application.api.application.auth.v1.UpdateAuthStatusRequest
	6,  // 6: application.api.application.auth.v1.Auth.DeleteAuth:input_type -> application.api.application.auth.v1.DeleteAuthRequest
	7,  // 7: application.api.application.auth.v1.Auth.ListOAuth:input_type -> application.api.application.auth.v1.ListOAuthRequest
	8,  // 8: application.api.application.auth.v1.Auth.DeleteOAuth:input_type -> application.api.application.auth.v1.DeleteOAuthRequest
	9,  // 9: application.api.application.auth.v1.Auth.GenAuthCaptcha:input_type -> application.api.application.auth.v1.GenAuthCaptchaRequest
	10, // 10: application.api.application.auth.v1.Auth.OAuthLogin:input_type -> application.api.application.auth.v1.OAuthLoginRequest
	11, // 11: application.api.application.auth.v1.Auth.EmailLogin:input_type -> application.api.application.auth.v1.EmailLoginRequest
	12, // 12: application.api.application.auth.v1.Auth.PasswordLogin:input_type -> application.api.application.auth.v1.PasswordLoginRequest
	13, // 13: application.api.application.auth.v1.Auth.EmailRegister:input_type -> application.api.application.auth.v1.EmailRegisterRequest
	14, // 14: application.api.application.auth.v1.Auth.PasswordRegister:input_type -> application.api.application.auth.v1.PasswordRegisterRequest
	15, // 15: application.api.application.auth.v1.Auth.EmailBind:input_type -> application.api.application.auth.v1.EmailBindRequest
	16, // 16: application.api.application.auth.v1.Auth.PasswordBind:input_type -> application.api.application.auth.v1.PasswordBindRequest
	17, // 17: application.api.application.auth.v1.Auth.RefreshToken:output_type -> application.api.application.auth.v1.RefreshTokenReply
	18, // 18: application.api.application.auth.v1.Auth.Logout:output_type -> application.api.application.auth.v1.LogoutReply
	19, // 19: application.api.application.auth.v1.Auth.Auth:output_type -> application.api.application.auth.v1.AuthReply
	20, // 20: application.api.application.auth.v1.Auth.ListAuth:output_type -> application.api.application.auth.v1.ListAuthReply
	21, // 21: application.api.application.auth.v1.Auth.CreateAuth:output_type -> application.api.application.auth.v1.CreateAuthReply
	22, // 22: application.api.application.auth.v1.Auth.UpdateAuthStatus:output_type -> application.api.application.auth.v1.UpdateAuthStatusReply
	23, // 23: application.api.application.auth.v1.Auth.DeleteAuth:output_type -> application.api.application.auth.v1.DeleteAuthReply
	24, // 24: application.api.application.auth.v1.Auth.ListOAuth:output_type -> application.api.application.auth.v1.ListOAuthReply
	25, // 25: application.api.application.auth.v1.Auth.DeleteOAuth:output_type -> application.api.application.auth.v1.DeleteOAuthReply
	26, // 26: application.api.application.auth.v1.Auth.GenAuthCaptcha:output_type -> application.api.application.auth.v1.GenAuthCaptchaReply
	27, // 27: application.api.application.auth.v1.Auth.OAuthLogin:output_type -> application.api.application.auth.v1.OAuthLoginReply
	28, // 28: application.api.application.auth.v1.Auth.EmailLogin:output_type -> application.api.application.auth.v1.EmailLoginReply
	29, // 29: application.api.application.auth.v1.Auth.PasswordLogin:output_type -> application.api.application.auth.v1.PasswordLoginReply
	30, // 30: application.api.application.auth.v1.Auth.EmailRegister:output_type -> application.api.application.auth.v1.EmailRegisterReply
	31, // 31: application.api.application.auth.v1.Auth.PasswordRegister:output_type -> application.api.application.auth.v1.PasswordRegisterReply
	32, // 32: application.api.application.auth.v1.Auth.EmailBind:output_type -> application.api.application.auth.v1.EmailBindReply
	33, // 33: application.api.application.auth.v1.Auth.PasswordBind:output_type -> application.api.application.auth.v1.PasswordBindReply
	17, // [17:34] is the sub-list for method output_type
	0,  // [0:17] is the sub-list for method input_type
	0,  // [0:0] is the sub-list for extension type_name
	0,  // [0:0] is the sub-list for extension extendee
	0,  // [0:0] is the sub-list for field type_name
}

func init() { file_api_application_auth_application_auth_service_proto_init() }
func file_api_application_auth_application_auth_service_proto_init() {
	if File_api_application_auth_application_auth_service_proto != nil {
		return
	}
	file_api_application_auth_application_auth_proto_init()
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_api_application_auth_application_auth_service_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   0,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_api_application_auth_application_auth_service_proto_goTypes,
		DependencyIndexes: file_api_application_auth_application_auth_service_proto_depIdxs,
	}.Build()
	File_api_application_auth_application_auth_service_proto = out.File
	file_api_application_auth_application_auth_service_proto_rawDesc = nil
	file_api_application_auth_application_auth_service_proto_goTypes = nil
	file_api_application_auth_application_auth_service_proto_depIdxs = nil
}
