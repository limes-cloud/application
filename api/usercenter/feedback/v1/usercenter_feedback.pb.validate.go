...
func (m *DeleteFeedbackRequest) validate(all bool) error {
	...
	_DeleteFeedbackRequest_Ids_Unique := make(map[uint32]struct{}, len(m.GetIds()))

	for idx, item := range m.GetIds() {
		_, _ = idx, item

		if _, exists := _DeleteFeedbackRequest_Ids_Unique[item]; exists {
			err := DeleteFeedbackRequestValidationError{
			...
			_DeleteFeedbackRequest_Ids_Unique[item] = struct{}{}
		}
