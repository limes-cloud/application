...
func (m *DeleteFeedbackCategoryRequest) validate(all bool) error {
	...
	_DeleteFeedbackCategoryRequest_Ids_Unique := make(map[uint32]struct{}, len(m.GetIds()))

	for idx, item := range m.GetIds() {
		_, _ = idx, item

		if _, exists := _DeleteFeedbackCategoryRequest_Ids_Unique[item]; exists {
			err := DeleteFeedbackCategoryRequestValidationError{
			...
			_DeleteFeedbackCategoryRequest_Ids_Unique[item] = struct{}{}
		}
